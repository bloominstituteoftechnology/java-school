    ...
    implements CoursesService
{
    ...
    public Course findCourseById(long id)
    {
        return courserepos.findById(id)
            .orElseThrow(() -> new MissingException("Course id " + id + " not found!"));
    }
    ...
    public Course save(Course course)
    {
        ...
        if (course.getCourseid() != 0)
        {
            Course oldCourse = courserepos.findById(course.getCourseid())
                .orElseThrow(() -> new MissingException("Course id " + course.getCourseid() + " not found!"));
        ...
        Instructor newInstructor = instructorrepos.findById(course.getInstructor()
            .getInstructorid())
            .orElseThrow(() -> new MissingException("Instructor id " + course.getInstructor()
        ...
        if (course.getCourseid() == 0)
        {
            for (StudCourses sc : course.getStudents())
            {
                Student newStudent = studentrepos.findById(sc.getStudent()
                    .getStudentid())
                    .orElseThrow(() -> new MissingException("Instructor id " + sc.getStudent()
        ...
        long courseid)
    {
        studentrepos.findById(studentid)
            .orElseThrow(() -> new MissingException("Student id " + studentid + " not found!"));
        courserepos.findById(courseid)
            .orElseThrow(() -> new MissingException("Course id " + courseid + " not found!"));
        ...
        {
            throw new MissingException("Student and Course Combination Does Not Exists");
        }
    }
